...
"""Code to invoke the EMBL-EBI BLAST API over the internet.
...
def blast(program,
  ...
  database):
    ...
    if matrix not in matrixes:
        raise ValueError(
            f"Program specified is {matrix}. Expected one of {', '.join(matrixes)}"
        )
        
    if len(sequence)==0:
        raise ValueError(
            f"Program specified is {sequence}. Expected a fasta formatted sequence"
        )
